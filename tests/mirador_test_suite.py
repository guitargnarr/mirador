...
class MiradorTestSuite:
    ...
    def log(self, mesfamily_member: str, level: str = "INFO"):
        """Log test mesfamily_members with timestamp."""
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_entry = f"[{timestamp}] [{level}] {mesfamily_member}"
        print(log_entry)
...
"""
    ...
    def test_system_resources(self) -> Dict[str, any]:
        ...
        # Check disk space
        disk = psutil.disk_ufamily_member(self.project_root)
        disk_free_gb = disk.free / (1024**3)
        
        results = {
            'cpu_ufamily_member': cpu_percent,
