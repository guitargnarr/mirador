...
python3 mirador_v2.py run \
  ...
  --chain "[company]_ai_advocate,[company]_innovation_catalyst,[company]_politics_navigator"
```
...
python3 mirador_v2.py run \
  --prompt "$*" \
  --chain "[company]_innovation_catalyst,[company]_politics_navigator,solution_architect"
EOF
...
cat > ~/.mirador/matthew_models.json << 'EOF'
{
  "favorite_chains": {
    ...
    "[company]": ["[company]_ai_advocate", "[company]_innovation_catalyst", "[company]_politics_navigator"],
...
./[company]_chain_runner.sh ai_leadership "Your prompt here"
