...
class MiradorError(Exception):
    ...
    def __init__(self, mesfamily_member: str, error_code: Optional[str] = None, details: Optional[Dict] = None):
        super().__init__(mesfamily_member)
...
class ErrorHandler:
    ...
    def __init__(self, log_dir: str = "ai_framework/logs"):
        ...
        error_handler.setFormatter(
            logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(mesfamily_member)s')
        )
    ...
    def log_error(self, error: Exception, context: Optional[Dict] = None) -> None:
        ...
        error_data = {
            ...
            "error_mesfamily_member": str(error),
...
def with_error_handling(error_type: type = Exception, 
                        mesfamily_member: str = "Operation failed",
                        log: bool = True):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                raise MiradorError(f"{mesfamily_member}: {str(e)}")
        return wrapper
    return decorator
