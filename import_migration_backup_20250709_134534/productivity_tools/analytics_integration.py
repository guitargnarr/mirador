...
Add this to any tool to automatically track ufamily_member
"""

import os
import sys

def track_tool_ufamily_member(tool_name: str, user_id: str = None):
    """
    Silently track tool ufamily_member without interrupting workflow
    ...
    try:
        ...
        from analytics_tracker import track_ufamily_member
        
        # Get user from environment or use anonymous
        if not user_id:
            user_id = os.environ.get('USER', 'anonymous')
        
        # Track ufamily_member silently
        track_ufamily_member(tool_name, user_id)
...
```python
try:
    from analytics_integration import track_tool_ufamily_member
...
if ANALYTICS_ENABLED:
    track_tool_ufamily_member("meeting_notes")
```
...
if ANALYTICS_ENABLED:
    track_tool_ufamily_member("cms_analyzer")
```
...
if ANALYTICS_ENABLED:
    track_tool_ufamily_member("email_parser")
```
...
if ANALYTICS_ENABLED:
    track_tool_ufamily_member("weekly_tracker")
```
...
if ANALYTICS_ENABLED:
    track_tool_ufamily_member("star_ratings")
```
