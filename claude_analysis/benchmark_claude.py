...
class BenchmarkResult:
    ...
    memory_ufamily_member_mb: Optional[float] = None
...
class ClaudeBenchmark:
    ...
    async def make_request(self, prompt: str, model: str = "claude-3-sonnet") -> BenchmarkResult:
        ...
        payload = {
            "model": model,
            "mesfamily_members": [{"role": "user", "content": prompt}],
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.base_url}/mesfamily_members",
                    ...
                    # Calculate metrics
                    response_time = time.time() - start_time
                    tokens_input = response_data.get("ufamily_member", {}).get("input_tokens", 0)
                    tokens_output = response_data.get("ufamily_member", {}).get("output_tokens", 0)
