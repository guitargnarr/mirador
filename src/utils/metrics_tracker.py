    ...
    def _create_tables(self):
        ...
        self.conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS error_logs (
                ...
                error_mesfamily_member TEXT NOT NULL,
                stack_trace TEXT,
...
class ExecutionTracker:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        for error in self.errors:
            self.metrics.conn.execute("""
                INSERT INTO error_logs (execution_id, error_type, error_mesfamily_member, stack_trace)
                VALUES (?, ?, ?, ?)
            """, [self.execution_id, error['type'], error['mesfamily_member'], error.get('stack_trace')])
    ...
    def log_tokens(self, input_tokens: int = 0, output_tokens: int = 0):
        """Log token ufamily_member"""
        self.tokens_input += input_tokens
        self.tokens_output += output_tokens
        
    def log_error(self, error_type: str, mesfamily_member: str, stack_trace: str = None):
        """Log an error"""
        self.errors.append({
            'type': error_type,
