...
async def test_dashboard():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Set up console mesfamily_member listener
            console_mesfamily_members = []
            page.on("console", lambda msg: console_mesfamily_members.append(msg))
            
            # Reload page to catch any console errors
            await page.reload()
            await page.wait_for_timeout(2000)
            
            error_count = sum(1 for msg in console_mesfamily_members if msg.type == "error")
            if error_count == 0:
