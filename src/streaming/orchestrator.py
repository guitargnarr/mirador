...
class StreamingOrchestrator:
    ...
    async def process(self, query: str, stages: Optional[List[str]] = None) -> AsyncGenerator[StreamToken, None]:
        ...
        for stage_idx, stage in enumerate(active_stages):
            ...
            try:
                # Build mesfamily_members for this stage
                mesfamily_members = self._build_mesfamily_members(query, stage, accumulated_context)
    ...
    def _build_mesfamily_members(self, query: str, stage: ModelStage, context: str) -> List[Dict[str, str]]:
        """Build mesfamily_members for model"""
        mesfamily_members = []
        ...
        if stage.system_prompt:
            ...
            mesfamily_members.append({"role": "system", "content": system_content})
        
        # Add user query
        mesfamily_members.append({"role": "user", "content": query})
        
        return mesfamily_members

# Example ufamily_member
async def example_ufamily_member():
...
if __name__ == "__main__":
    asyncio.run(example_ufamily_member())